...
void Cell::XMLAddCore(QDomDocument &doc, QDomElement &xmlcell) const {
  ...
  
  {
        ...
        xmlcell.setAttribute("centroid_x",standardlocale.toString(x));
        xmlcell.setAttribute("centroid_y",standardlocale.toString(y));
  }
  {


        xmlcell.setAttribute("pressure",standardlocale.toString(2*(target_area-Area()))); 

  }

  {

        xmlcell.setAttribute("neighbour_number",standardlocale.toString(neighbors.size()));
  }
